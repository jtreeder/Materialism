...
PREV_JSON = OUT_JSON
TABLE_A1_CSV = os.path.join(SCRIPT_DIR, "..", "data", "processed", "table_a1.csv")
...
def generate_name_guesses(name):
    ...
    # ── ACD-style hyphenation → IUPAC-style ─────────────────────────────
    # ACD names hyphenate before the parent chain: "1-Bromo-2-chloro-ethene"
    # PubChem expects IUPAC where the last substituent joins the parent:
    #   "1-Bromo-2-chloro-ethene" → "1-Bromo-2-chloroethene"
    #   "2-(2-Hexyloxy-ethoxy)-hexanol" → "2-(2-Hexyloxyethoxy)-hexanol"
    # Strategy: find the last hyphen before a lowercase parent-chain word
    # (not preceded by a digit or comma) and remove it.
    _parent_chains = (
        ...
        "benzene", "toluene", "pyridine", "furan", "thiophene",
        "amine", "amide", "ether", "ester", "acid", "aldehyde",
        "acetate", "acrylate", "anhydride",
    )
    for parent in _parent_chains:
        pat = re.compile(r"-(" + re.escape(parent) + r")\b", re.IGNORECASE)
        m2 = pat.search(o)
        if m2:
            fixed = o[:m2.start()] + m2.group(1) + o[m2.end():]
            if fixed != o:
                _add(guesses, fixed, f"Join substituent to parent chain '{parent}'", 0.90)
                break
...
def _clean_acd_name(acd):
    """Clean OCR artifacts from an ACD name (line-break hyphens, extra spaces)."""
    if not acd:
        return acd
...
def _load_acd_names():
    """Load ACD/Autonom names, preferring table_a1.csv over previous JSON.

    table_a1.csv is parsed from pdftotext OCR by clean_hsp.py, which
    correctly handles multi-line name continuations.  The previous JSON's
    source_ref.acd_name may be truncated for multi-line entries because
    PyMuPDF extraction only captures a single y-coordinate row.
    """
    # Primary: load from table_a1.csv keyed by solvent name
    a1_by_name = {}
    if os.path.exists(TABLE_A1_CSV):
        try:
            with open(TABLE_A1_CSV) as f:
                for row in csv.DictReader(f):
                    sname = row.get("solvent_name", "").strip()
                    acd = row.get("autonom_acd_name", "").strip()
                    if sname and acd:
                        a1_by_name[sname] = _clean_acd_name(acd)
        except (csv.Error, KeyError):
            pass

    # Build row_index → acd_name map.
    # First load any from previous JSON, then override with table_a1 data.
    ...
    # Override with table_a1 ACD names (matched by original_name)
    # We need to map row_index → name first via the chemicals CSV
    if a1_by_name and os.path.exists(CHEM_CSV):
        with open(CHEM_CSV) as f:
            for i, row in enumerate(csv.DictReader(f)):
                if not row.get("cas_number", "").strip():
                    name = row["name"].strip()
                    if name in a1_by_name:
                        acd_map[i] = a1_by_name[name]
