...
<body>
    ...
    <script>
        ...
        function _updatePlotForCommonFilter() {{
            ...
            if (simpleMode) {{
                // Hide non-common materials (fully transparent) but keep marker sizes unchanged
                var filteredColors = SOLVENTS.map(function(s, i) {{
                    return s.common ? (_solventColors[i] || '#888') : 'rgba(0,0,0,0)';
                }});
                var filteredOpacity = SOLVENTS.map(function(s) {{ return s.common ? 0.9 : 0; }});
                var filteredPolyColors = POLYMERS.map(function(p) {{ return p.common ? 'gold' : 'rgba(0,0,0,0)'; }});
                var filteredPolyOpacity = POLYMERS.map(function(p) {{ return p.common ? 0.95 : 0; }});
        ...
        document.addEventListener('DOMContentLoaded', function() {{
            ...
            buildFullPlot();
            buildHomeTable();
            // Native Plotly hover handles marker tooltips; annotations for pin + table hover
