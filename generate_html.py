...
<body>
    ...
    <script>
        ...
        // Dynamic color maps — rebuilt when active datasets change so colors
        // always spread across the full spectrum for maximum visibility.
        var _dynamicCatColors = {{}};
        var _dynamicPolyCatColors = {{}};

        function _generateDistinctColors(n, hueOffset) {{
            if (n === 0) return [];
            var colors = [];
            var goldenAngle = 137.508; // degrees — optimal irrational spacing
            hueOffset = hueOffset || 0;
            for (var i = 0; i < n; i++) {{
                var hue = (hueOffset + i * goldenAngle) % 360;
                var sat = 65 + (i % 3) * 12;   // 65%, 77%, 89%
                var light = 50 + (i % 2) * 12;  // 50%, 62%
        ...
        function _computeDynamicColors() {{
            // Determine which categories exist in active datasets
            var solCats = {{}};
            SOLVENTS.forEach(function(s) {{
                if (!_isDsActive(s.dsId)) return;
                var c = s.cat || 'other';
                solCats[c] = (solCats[c] || 0) + 1;
            }});
            var polyCats = {{}};
            POLYMERS.forEach(function(p) {{
                if (!_isDsActive(p.dsId)) return;
                var c = p.cat || 'Other';
                polyCats[c] = (polyCats[c] || 0) + 1;
            }});
            // Sort by count descending for consistent ordering
            var solList = Object.keys(solCats).sort(function(a,b) {{ return solCats[b] - solCats[a]; }});
            var polyList = Object.keys(polyCats).sort(function(a,b) {{ return polyCats[b] - polyCats[a]; }});
            // Generate colors spread across full spectrum
            // Solvent hues start at 0°, polymer hues offset by 60° to avoid collisions
            var solColors = _generateDistinctColors(solList.length, 0);
            var polyColors = _generateDistinctColors(polyList.length, 60);
            _dynamicCatColors = {{}};
            solList.forEach(function(cat, i) {{ _dynamicCatColors[cat] = solColors[i]; }});
            _dynamicPolyCatColors = {{}};
            polyList.forEach(function(cat, i) {{ _dynamicPolyCatColors[cat] = polyColors[i]; }});
        ...
        function _buildLegend() {{
            ...
            var firstSolColor = sList.length > 0 ? (_dynamicCatColors[sList[0]] || '#888') : '#888';
            ...
            h += '<span class="legend-marker" style="background:' + firstSolColor + '"></span>';
            ...
            var firstPolyColor = pList.length > 0 ? (_dynamicPolyCatColors[pList[0]] || '#a9a9a9') : '#a9a9a9';
            ...
            h += '<span class="legend-arrow" onclick="event.stopPropagation();toggleLegendGroup(this.parentNode)">&#9654;</span>';
            h += '<span class="legend-marker diamond" style="background:' + firstPolyColor + '"></span>';
