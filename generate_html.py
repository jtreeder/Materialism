...
<body>
    ...
    <script>
        ...
        function _updatePlotForCommonFilter() {{
            ...
            // Update each polymer category trace
            for (var ti = 0; ti < _polyTraceCount; ti++) {{
                var trace = plotDiv.data[_polyTraceStart + ti];
                ...
                trace.x = px; trace.y = py; trace.z = pz;
                trace.marker.size = pSize;
                trace.marker.color = _plotDimmed ? '#665500' : trColor;
                trace.marker.opacity = pOpacity;
                trace.hoverinfo = hInfo;
            }}
