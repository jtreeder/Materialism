    ...
    for (var i = 0; i < queue.length; i++) {{
        ...
        try {{
            ...
            if (entry.missing.indexOf('commercial_products') !== -1) {{
                ...
                if (cpCid || cpName) {{
                    _setStep(_iName, _iNum, queue.length, 'Searching for commercial products (Use & Manufacturing data)...', true);
                    await _delay(300);
