...
POLYMER_TYPE_TO_CAT = {
    ...
    # hsp_polymers_9 lowercase type names from seed table classification
    "cellulosic": "Cellulose",
    "fluoropolymer": "Fluoropolymer",
    "fluoroelastomer": "Fluoropolymer",
    "fluoropolymer coating resin": "Fluoropolymer",
    "polyolefin": "Polyolefin",
    "styrenic": "Vinyl & Styrene",
    "acrylic": "Acrylic",
    "acrylic copolymer": "Acrylic",
    "chlorinated polymer": "Halogenated",
    "chlorinated rubber": "Rubber & Elastomer",
    "polyester": "Polyester & Alkyd",
    "polyester resin": "Polyester & Alkyd",
    "alkyd resin": "Polyester & Alkyd",
    "polyamide": "Polyamide & Imide",
    "polyamide resin": "Polyamide & Imide",
    "polycarbonate": "Engineering",
    "polyacetal": "Engineering",
    "engineering plastic": "Engineering",
    "polyurethane": "Urethane",
    "polyurethane precursor": "Urethane",
    "epoxy resin": "Epoxy",
    "epoxy hardener": "Epoxy",
    "thermoset": "Resin",
    ...
    "thermoplastic elastomer": "Rubber & Elastomer",
    "ionomer": "Vinyl & Styrene",
    "copolymer": "Other",
    "polyvinyl acetal": "Vinyl & Styrene",
    "polyvinyl alcohol": "Vinyl & Styrene",
    "natural resin": "Natural & Bio",
    "natural oil": "Natural & Bio",
    "animal fat": "Natural & Bio",
    "non-polymer": "Other",
    "condition entry": "Other",
    "resistance data": "Other",
    "Acrylic": "Acrylic",
...
<body>
    ...
    <script>
        ...
        var _lastSolventDimmed = null; // tracks last dimmed state applied to solvent size
        ...
        function _updatePlotForCommonFilter() {{
            ...
            // Solvent trace (trace 0) — always use real coordinates to avoid
            // Plotly 3D re-orienting axis titles when the data extent changes.
            // Hidden points are made invisible via transparent color.
            // IMPORTANT: never restyle marker.size on the solvent trace except
            // when the dimmed state changes — any restyle of size (even to the
            // same scalar value) causes scatter3d WebGL to render markers smaller.
            var sx = [], sy = [], sz = [], sc = [];
            ...
            var solData = {{
                'x': [sx], 'y': [sy], 'z': [sz],
                'marker.color': [_plotDimmed ? sc.map(function(c) {{ return c === 'rgba(0,0,0,0)' ? 'rgba(0,0,0,0)' : '#999'; }}) : sc],
                'marker.opacity': [sOpacity], 'hoverinfo': [hInfo],
            }};
            // Only touch marker.size when dim state changes to avoid WebGL size glitch
            if (_plotDimmed !== _lastSolventDimmed) {{
                solData['marker.size'] = [sSize];
                _lastSolventDimmed = _plotDimmed;
            }}
            Plotly.restyle(plotDiv, solData, [0]);

            // Polymer category traces
            var polyIndices = [], polyX = [], polyY = [], polyZ = [];
            var polySizes = [], polyColors = [], polyOpacities = [], polyHInfos = [];
            for (var ti = 0; ti < _polyTraceCount; ti++) {{
                ...
                polyIndices.push(_polyTraceStart + ti);
                polyX.push(px); polyY.push(py); polyZ.push(pz);
                polySizes.push(pSize);
                polyColors.push(_plotDimmed ? '#665500' : trColor);
                polyOpacities.push(pOpacity);
                polyHInfos.push(hInfo);
            }}
            if (polyIndices.length > 0) {{
                Plotly.restyle(plotDiv, {{
                    'x': polyX, 'y': polyY, 'z': polyZ,
                    'marker.size': polySizes, 'marker.color': polyColors,
        ...
        function _getActiveRanges() {{
            ...
            if (!isFinite(maxD)) return DEFAULT_AXES;
            return {{
                xRange: [Math.floor(minD / 5) * 5, Math.ceil(maxD / 5) * 5 || 5],
                yRange: [Math.floor(minP / 5) * 5, Math.ceil(maxP / 5) * 5 || 5],
                zRange: [Math.floor(minH / 5) * 5, Math.ceil(maxH / 5) * 5 || 5],
            }};
        }}
        ...
        function runSearch() {{
            ...
            if (!q) return;
            // Clear any active isolation so the full dataset is visible in results
            if (_isolatedItems) {{
                _isolatedItems = null;
                _applyIsolation();
            }}
...
(function() {{
    try {{
        ...
        Object.keys(imported).forEach(function(dsId) {{
            ...
            (ds.chemicals || []).forEach(function(c) {{
                SOLVENTS.push({{
                    name: c.name || c.name_common || '', cas: c.cas || '', smiles: c.smiles || '',
                    formula: c.formula || '', dd: c.dd || '', dp: c.dp || '', dh: c.dh || '',
                    mw: c.mw || '', bp: c.bp || '', density: c.density || '',
...
function renderDetail() {{
    ...
    html += '<h2>' + (m.name || dsId) + '<button onclick="toggleEditMode(\\x27' + dsId + '\\x27)" title="' + (isEdit ? 'Click to lock' : 'Click to unlock editing') + '" style="background:none;border:none;cursor:pointer;padding:0 0 2px 8px;vertical-align:middle;color:' + (isEdit ? '#e94560' : '#b2bec3') + ';line-height:1"><svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">' + (isEdit ? _LOCK_ICON_UNLOCKED : _LOCK_ICON_LOCKED) + '</svg></button></h2>';
    ...
    if (_sortCol === -1) {{
        filtered = filtered.slice().sort(function(a,b) {{
            var ea = _isExcluded(itemType, dsId, a.name||a.name_common||'') ? 0 : 1;
            var eb = _isExcluded(itemType, dsId, b.name||b.name_common||'') ? 0 : 1;
            return _sortAsc ? ea - eb : eb - ea;
        }});
    ...
    for (var i = 0; i < limit; i++) {{
        ...
        var _exclKey = (dsId + ':' + itemType + ':' + rName).replace(/"/g,'&quot;');
        html += '<td style="text-align:center;padding:0 4px"><input type="checkbox"' + (rChecked ? ' checked' : '') + ' data-excl-key="' + _exclKey + '" onchange="_toggleExclusion(\\x27' + itemType + '\\x27,\\x27' + dsId + '\\x27,' + JSON.stringify(rName).replace(/"/g,'&quot;') + ',this);this.closest(\\x27tr\\x27).style.opacity=this.checked?\\x271\\x27:\\x270.45\\x27"></td>';
        cols.forEach(function(c) {{
            ...
            var catStyle = '';
            var display = v;
            if ((c === 'name' || c === 'name_common' || c === 'name_iupac') && (v || r.name)) {{
                ...
                display = '<a class="cas-link" href="https://pubchem.ncbi.nlm.nih.gov/#query=' + encodeURIComponent(v) + '" target="_blank" rel="noopener">' + v + '</a>';
            }} else if (c === 'cat' && v) {{
    ...
    ct.innerHTML = html;
    // Force correct checkbox states — overrides any browser form-state restoration
    ct.querySelectorAll('input[type="checkbox"][data-excl-key]').forEach(function(cb) {{
        var shouldBeChecked = !_excludedItems[cb.getAttribute('data-excl-key')];
        cb.checked = shouldBeChecked;
        cb.closest('tr').style.opacity = shouldBeChecked ? '1' : '0.45';
    }});
