...
<div class="main">
    <div class="sidebar" id="sidebar">
        ...
        <div id="ds-list"></div>
        <button class="rebuild-btn" onclick="triggerRebuild()">Rebuild Database &amp; HTML</button>
    </div>
...
// --- API base URL — auto-detect from page origin, fallback to localhost:8050 ---
var API_BASE = (window.location.protocol === 'file:' || !window.location.host)
    ? 'http://localhost:8050'
...
var _pendingAnalysis = null; // stores last analysis report
...
function uploadFile(file) {{
    ...
    var fd = new FormData();
    fd.append('file', file);
    _apiFetch('/api/datasets/upload', {{ method: 'POST', body: fd }})
        .then(function(data) {{
            if (data.error) {{ ct.innerHTML = '<div class="analysis-card"><h3>Error</h3><p>' + data.error + '</p></div>'; return; }}
            data.original_filename = data.original_filename || file.name;
            _pendingAnalysis = data;
            showAnalysis(data);
        }})
        .catch(function(err) {{ ct.innerHTML = '<div class="analysis-card"><h3>Error</h3><p>' + err + '</p></div>'; }});
}}
...
function analyzeUrl() {{
    ...
    if (!url) return;
    var ct = document.getElementById('content');
    ct.innerHTML = '<div class="loading">Fetching and analyzing URL...</div>';
    _apiFetch('/api/datasets/analyze-url', {{ method: 'POST', headers: {{'Content-Type': 'application/json'}}, body: JSON.stringify({{url: url}}) }})
        .then(function(data) {{
            if (data.error) {{ ct.innerHTML = '<div class="analysis-card"><h3>Error</h3><p>' + data.error + '</p></div>'; return; }}
            _pendingAnalysis = data;
            showAnalysis(data);
...
function searchDatabases() {{
    ...
    _apiFetch('/api/datasets/search', {{ method: 'POST', headers: {{'Content-Type': 'application/json'}}, body: JSON.stringify({{query: q}}) }})
        .then(function(data) {{
            if (data.error) {{ ct.innerHTML = '<div class="analysis-card"><h3>Error</h3><p>' + data.error + '</p></div>'; return; }}
...
function doImport() {{
    ...
    var body = {{
        analysis_id: _pendingAnalysis.analysis_id,
        dataset_id: dsId,
        name: document.getElementById('import-name').value.trim() || dsId,
        source_url: document.getElementById('import-url').value.trim(),
        confidence_tier: parseFloat(document.getElementById('import-conf').value) || 0.30,
        column_mapping: mapping,
    }};

    var ct = document.getElementById('content');
    ct.innerHTML = '<div class="loading">Importing dataset...</div>';

    _apiFetch('/api/datasets/import', {{ method: 'POST', headers: {{'Content-Type': 'application/json'}}, body: JSON.stringify(body) }})
        .then(function(data) {{
            if (data.error) {{ ct.innerHTML = '<div class="analysis-card"><h3>Import Error</h3><p>' + data.error + '</p></div>'; return; }}
            var ds = data.dataset;
            // Add to local DATASETS object
            DATASETS[ds.id] = {{ chemicals: [], polymers: [], meta: ds }};
            _activeDsets[ds.id] = true;
            _saveA(_activeDsets);
            ct.innerHTML = '<div class="analysis-card"><h3>Imported!</h3><p>' + ds.chemical_count + ' chemicals, ' + ds.polymer_count + ' polymers imported as <b>' + ds.id + '</b>.</p><p style="margin-top:8px;font-size:0.85rem;color:#636e72">Click <b>Rebuild Database &amp; HTML</b> in the sidebar to update the visualization and database pages.</p></div>';
...
function triggerRebuild() {{
    var ct = document.getElementById('content');
    ct.innerHTML = '<div class="loading">Rebuilding database and HTML...</div>';
    _apiFetch('/api/build', {{ method: 'POST' }})
        .then(function(data) {{
            if (data.error) {{ ct.innerHTML = '<div class="analysis-card"><h3>Build Error</h3><p>' + data.error + '</p></div>'; return; }}
            ct.innerHTML = '<div class="analysis-card"><h3>Build Complete</h3><pre style="font-size:0.75rem;max-height:300px;overflow:auto;background:#f5f6fa;padding:8px;border-radius:4px">' + (data.build_output||'') + '</pre><p style="margin-top:8px;font-size:0.85rem;color:#636e72">Refresh database.html and materialism.html to see the changes.</p></div>';
        }})
        .catch(function(err) {{ ct.innerHTML = '<div class="analysis-card"><h3>Error</h3><p>' + err + '</p></div>'; }});
}}
