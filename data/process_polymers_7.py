#!/usr/bin/env python3
"""
process_polymers_7.py — HSP Polymer pipeline v7
Source: data/raw/a2only.pdf (Table A.2 — 466 polymer entries)

New vs v5:
- Extracts directly from PDF (pdfplumber)
- Applies Step 0B OCR corrections
- Adds Step 0E data_type / data_reliability columns
- Dataset ID: hsp_polymers_7
"""
...
PDF_PATH    = BASE / "raw/a2only.pdf"
OUT_DIR     = BASE / "datasets/hsp_polymers_7"
OUT_DIR.mkdir(parents=True, exist_ok=True)
ENRICHED    = OUT_DIR / "polymers_enriched.csv"
DB_CSV      = OUT_DIR / "polymers.csv"
V5_ENRICHED = BASE / "datasets/hsp_polymers_5/polymers_enriched.csv"
...
# ─── STEP 0E: data_type / data_reliability lookup ─────────────────────────────

def build_source_range_map():
    ranges = [
        (range(1,110),   "solubility",          "high"),
        (range(110,111), "permeation",           "medium"),
        (range(111,113), "solubility",           "high"),
        (range(113,115), "chemical_resistance",  "medium"),
        (range(115,116), "solubility",           "high"),
        (range(116,117), "solubility",           "medium"),
        (range(117,141), "breakthrough_time",    "high"),
        (range(141,143), "permeation",           "high"),
        (range(143,145), "solubility",           "high"),
        (range(145,149), "chemical_resistance",  "medium"),
        (range(149,161), "chemical_resistance",  "medium"),
        (range(161,162), "swelling",             "medium"),
        (range(162,164), "swelling",             "high"),
        (range(164,168), "breakthrough_time",    "high"),
        (range(168,182), "chemical_resistance",  "medium"),
        (range(182,183), "solubility",           "medium"),
        (range(183,184), "solubility",           "high"),
        (range(184,187), "esc",                  "high"),
        (range(187,225), "estimated",            "low"),
        (range(225,347), "estimated",            "low"),
        (range(347,348), "solubility",           "high"),
        (range(348,349), "solubility",           "low"),
        (range(349,350), "solubility",           "high"),
        (range(350,362), "estimated",            "medium"),
        (range(362,363), "solubility",           "high"),
        (range(363,365), "chemical_resistance",  "medium"),
        (range(365,368), "solubility",           "medium"),
        (range(368,370), "permeation",           "high"),
        (range(370,372), "solubility",           "high"),
        (range(372,390), "chemical_resistance",  "medium"),
        (range(390,413), "chemical_resistance",  "medium"),
        (range(413,451), "concentration_series", "high"),
        (range(451,454), "swelling",             "medium"),
        (range(454,459), "chemical_resistance",  "medium"),
        (range(459,460), "solubility",           "high"),
        (range(460,462), "solubility",           "high"),
        (range(462,464), "solubility",           "medium"),
        (range(464,467), "solubility",           "medium"),
    ]
    m = {}
    for r, dt, dr in ranges:
        for n in r:
            m[n] = (dt, dr)
    return m

SOURCE_RANGE_MAP = build_source_range_map()


# ─── STEP 0A: PDF EXTRACTION ──────────────────────────────────────────────────

def extract_from_pdf(pdf_path: Path) -> pd.DataFrame:
    """Extract Table A.2 rows from the PDF. Returns DataFrame with No,Material,dD,dP,dH,Ro."""
    all_lines = []
    with pdfplumber.open(pdf_path) as pdf:
        for i in range(8, min(21, len(pdf.pages))):
            page = pdf.pages[i]
    ...
    df = pd.DataFrame(rows).sort_values("No").reset_index(drop=True)
    print(f"Extracted {len(df)} rows from PDF")

    # Check for gaps
    found = set(df["No"].tolist())
    missing = sorted(set(range(1, 467)) - found)
    if missing:
        print(f"  Missing rows: {missing}")
    else:
        print("  No missing rows (all 466 present)")

    return df


# ─── STEP 0B: OCR CORRECTIONS ────────────────────────────────────────────────

# Name corrections (row No → corrected name)
NAME_CORRECTIONS = {
    5:   "ARALDITE DY-025",
    35:  "LUTONAL I60",
    48:  "1/2-sec.-NITROCELLULOSE H 23",
    61:  "BUTVAR B 76",
...
# HSP corrections (row No → (dD, dP, dH, Ro))
HSP_CORRECTIONS = {
    238: (28.50, 15.70, 18.10, 21.50),
    239: (25.50, 15.70, 18.10, 21.50),
    369: (15.30, 5.30, 2.50, 10.10),
}

# Ro artifact rows — round to 2 dp (these may already be clean from PDF, but apply anyway)
RO_ARTIFACT_ROWS = {31, 55, 89, 128, 170, 212, 257, 302, 346, 382, 426, 459}
# Row 466 special case
RO_466_VALUE = 3.90
...
def apply_ocr_corrections(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, row in df.iterrows():
        ...
        # Name corrections
        if no in NAME_CORRECTIONS:
            old = df.at[idx, "Material"]
            new = NAME_CORRECTIONS[no]
            if old != new:
                df.at[idx, "Material"] = new
                row_notes.append(f"[OCR] name: {old!r} → {new!r}")

        # HSP corrections
        if no in HSP_CORRECTIONS:
            dd, dp, dh, ro = HSP_CORRECTIONS[no]
            df.at[idx, "dD"] = dd
            df.at[idx, "dP"] = dp
            df.at[idx, "dH"] = dh
            df.at[idx, "Ro"] = ro
            row_notes.append(f"[OCR] HSP column-shift corrected")

        # Ro artifact stripping
        if no in RO_ARTIFACT_ROWS:
            val = round(df.at[idx, "Ro"], 2)
            df.at[idx, "Ro"] = val
            row_notes.append("[OCR] Ro rounded to 2dp")

        if no == 466:
            df.at[idx, "Ro"] = RO_466_VALUE

        # Collapse double spaces in name
        nm = re.sub(r'\s{2,}', ' ', df.at[idx, "Material"]).strip()
        if nm != df.at[idx, "Material"]:
            df.at[idx, "Material"] = nm
            row_notes.append("[OCR] double space collapsed")

        notes[idx] = "; ".join(row_notes)

    df["ocr_notes"] = df.index.map(lambda i: notes.get(i, ""))
    return df
...
def load_v5_cas() -> dict:
    """Load CAS results from v5 enriched CSV to avoid re-querying PubChem."""
    if not V5_ENRICHED.exists():
        return {}
    try:
        df = pd.read_csv(V5_ENRICHED, dtype=str)
        cas_map = {}
        for _, row in df.iterrows():
            ni = str(row.get("name_input", "")).upper().strip()
            cas = str(row.get("cas", "")).strip()
            if ni and cas and cas not in ("", "nan", "None"):
                cas_map[ni] = cas
        print(f"Loaded {len(cas_map)} CAS entries from v5 cache")
        return cas_map
    except Exception as e:
        print(f"Warning: could not load v5 CAS cache: {e}")
        return {}
...
def run():
    print("=" * 60)
    print("HSP Polymers 7 — Pipeline")
    print("Source: data/raw/a2only.pdf (Table A.2)")
    print("=" * 60)

    # Step 0A: Extract from PDF
    df = extract_from_pdf(PDF_PATH)
    print(f"Rows extracted: {len(df)}")

    # Step 0B: OCR corrections
    df = apply_ocr_corrections(df)
    ocr_count = (df["ocr_notes"] != "").sum()
    print(f"OCR corrections applied: {ocr_count} rows modified")
    ...
    for idx, row in df.iterrows():
        name_input = row["Material"]
        name_upper = name_input.upper().strip()
        no = int(row["No"])
        notes_parts = []

        # Carry over OCR notes
        if row.get("ocr_notes"):
            notes_parts.append(row["ocr_notes"])

        # Step 0E: data_type and data_reliability
        dt, dr = SOURCE_RANGE_MAP.get(no, ("unknown", "medium"))
        if no not in SOURCE_RANGE_MAP:
            notes_parts.append(f"[DATA_TYPE] no source range defined for row {no}")
        ...
        # Step 2: HSP anomaly flags
        hsp_row = {"dD": row.get("dD"), "dP": row.get("dP"), "dH": row.get("dH"), "radius": row.get("Ro")}
        ...
        if name_upper in v5_cas:
            cas = v5_cas[name_upper]
            notes_parts.append(f"[CAS] from v5 cache: {cas}")
            cas_found += 1

        # PubChem from acronym table CID
        nc_upper = resolved["name_clean"].upper()
        if not cas and nc_upper in ACRONYM_TABLE and ACRONYM_TABLE[nc_upper][2]:
            pubchem_cid = str(ACRONYM_TABLE[nc_upper][2])
            notes_parts.append(f"[CAS] PubChem CID from acronym table: {pubchem_cid}")

        if not cas and not pubchem_cid and pre.get("route") not in (
            "trade_name", "time_series", "resistance_data",
            "concentration_series", "flag_out_of_scope"
        ):
    ...
    # Write polymers.csv (database-ready)
    db_rows = []
    src_url = "https://github.com/jtreeder/Materialism/blob/claude/hansen-solubility-planning-D5iok/data/raw/a2only.pdf"
    for r in rows_out:
        db_rows.append({
            "name":              r["name_clean"],
            ...
            "confidence":        0.9,
            "source_count":      1,
            "source":            "hsp_polymers_7",
            "source_url":        src_url,
            "name_iupac":        r["name_iupac"],
            "name_common":       r["name_common"],
            "name_common_all":   r["name_common_all"],
            "name_acronyms":     r["name_acronyms"],
    ...
    meta = {
        ...
        "source_url": src_url,
        "description": (
            "Hansen Solubility Parameters Table A.2 extracted directly from "
            "the original Hansen (2007) PDF. Full pipeline: PDF extraction with "
            "pdfplumber, Step 0B OCR corrections, 13-class polymer classification, "
            "Step 0E data_type/data_reliability metadata, CAS lookup via PubChem, "
            "name enrichment (IUPAC, common, acronyms), processing audit trail."
        ),
        "imported_at": "2026-03-09T00:00:00+00:00",
        "chemical_count": 0,
        ...
        "fields_available": [
            ...
            "class","subclass","data_type","data_reliability","processing_notes"
        ],
        "quality_notes": (
            f"Pipeline v7: PDF source (a2only.pdf, 466 rows). "
            f"13-class classification (rule-based). "
            f"Step 0E data_type/data_reliability metadata. "
    ...
    dt_counts = enriched_df["data_type"].value_counts()
